...
2. **Dealer orders (`orders.csv`, `order_source=dealer`)** as part-level external demand.
3. **Install forecast (`install_forecast.csv`)** as model-driven demand adjustment:
...
2. Prepare internal usage history and part-model compatibility.
3. Detect outliers using Z-score threshold and preserve original values for traceability.
4. Compute usage-per-machine from usage and active machine population.
5. Build base demand forecast (exponential smoothing by default; Holt trend only when history and significance requirements are met).
6. Add dealer demand contribution at part level.
7. Apply install demand adjustment.
8. Produce final demand used by inventory logic.
