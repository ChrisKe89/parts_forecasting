...
def run_backtest(data: dict[str, pd.DataFrame], train_months: int = 21, test_months: int = 3) -> pd.DataFrame:
    ...
    actual = test_usage.groupby(["part_number"], as_index=False)["usage_qty"].sum().rename(columns={"usage_qty": "actual_usage"})
    merged = forecast_df.merge(actual, on=["part_number"], how="left").fillna({"actual_usage": 0})
